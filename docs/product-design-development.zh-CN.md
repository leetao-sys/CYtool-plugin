...
- 插件页面初版采用 iframe 隔离。
- 插件 zip 包允许包含并执行自定义 Python 后端代码。
- 插件需要平台分配私有数据目录。
- SSH 初版同时支持普通用户登录后 `su root` 和 `sudo -S`。
- 远程数据库先保留适配器接口，P0 完整落地 SQLite。

## 13. 仍需细化的问题

1. 插件自定义 Python 后端代码采用同进程加载、子进程运行，还是可配置两种模式？
2. 插件 Python 后端 API 是否需要热加载，还是安装/更新后重启平台生效即可？
3. 保存 SSH/数据库连接信息时，是否允许保存密码，还是每次执行时手动输入？
4. 第一版是否需要登录认证和管理员角色，还是先做单机可信环境？
